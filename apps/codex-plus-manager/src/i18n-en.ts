...
export const EN_PLAIN: Record<string, string> = {
  ...
  "对可恢复的请求错误执行自动重试。": "Automatically retry recoverable request errors.",
  "capacity 重试": "Capacity retry",
  "capacity 重试次数": "Capacity retry attempts",
  "检测到容量错误后，Codex++ 会在 Codex 内提示并在代理中重发原请求；中间错误不进入任务，也不计入 Codex 重试次数。达到上限后才展示原始容量错误。":
    "When a capacity error is detected, Codex++ shows a notice in Codex and reissues the original request inside the proxy. Intermediate errors never enter the task or count toward Codex retries; the original error is shown only after the limit is reached.",
  "只计算 Codex++ 内部重新发起请求的次数，不使用 Codex 自身的错误计数；可设置 1–20 次。":
    "Counts only requests reissued internally by Codex++, without using Codex's own error counter. Configurable from 1 to 20 retries.",
  ...
  "官方登录": "Official login",
  "官方登录混合模式": "Official login hybrid mode",
  "官方登录模式": "Official login mode",
  "官方账号只负责优先认证；选择第三方或聚合供应商后，由其 API 配置覆盖实际请求，官方 API 不加入聚合轮转。":
    "The official account authenticates first. A selected third-party or aggregate provider then overrides requests, while the official API stays outside aggregate rotation.",
  "认证账号": "Authentication account",
  "请求目标": "Request target",
  "当前使用官方 API；点击其他供应商的“使用”即可覆写请求目标。":
    "The official API is active. Click Use on another provider to override the request target.",
  "如需官方账号认证后再使用本聚合，请返回供应商列表开启“官方登录混合模式”；官方 API 不会加入成员轮转。":
    "To authenticate with an official account before using this aggregate, enable Official login hybrid mode in the provider list. The official API is not added to member rotation.",
  "只能勾选已填写 Base URL / Key 的第三方 API 供应商；官方登录与其他聚合供应商不会作为成员。":
    "Only third-party API providers with a Base URL and Key can be selected. Official login and other aggregate providers are not members.",
  "为官方模型设置成员供应商和真实上游模型。显示使用半角字符，例如 gpt-5.4(openai;供应商1|供应商2:真实模型)。":
    "Map official models to member providers and real upstream models. Labels use ASCII punctuation, for example gpt-5.4(openai;provider1|provider2:real-model).",
  ...
  "CLIProxyAPI 状态": "CLIProxyAPI status",
  "CLIProxyAPI 管理页面": "CLIProxyAPI management",
  "CLIProxyAPI 模型": "CLIProxyAPI models",
  ...
  "停止 CLIProxyAPI": "Stop CLIProxyAPI",
  "重启 CLIProxyAPI": "Restart CLIProxyAPI",
  "NewAPI 状态": "NewAPI status",
  "NewAPI 管理页面": "NewAPI management",
  "NewAPI 模型": "NewAPI models",
  "NewAPI 测试": "NewAPI test",
  "NewAPI API Token": "NewAPI API token",
  "NewAPI 启动与连接位置": "NewAPI launch and connection settings",
  "NewAPI 供应商": "NewAPI provider",
  "启动 NewAPI": "Start NewAPI",
  "停止 NewAPI": "Stop NewAPI",
  "重启 NewAPI": "Restart NewAPI",
  "NewAPI 接入": "NewAPI integration",
  "NewAPI 未返回可接入模型，请先配置渠道与用户令牌后刷新模型。":
    "NewAPI returned no models to integrate. Configure channels and a user token, then refresh models.",
  "已停止": "Stopped",
  "未配置": "Not configured",
  "已复制到剪贴板。": "Copied to the clipboard.",
  "运行正常": "Running",
  "运行异常": "Unhealthy",
  "API 未就绪": "API not ready",
  "项目未配置": "Project not configured",
  "未找到 Docker": "Docker not found",
  "Docker 未运行": "Docker is not running",
  "Compose 不可用": "Docker Compose unavailable",
  "未找到": "Not found",
  "可用": "Available",
  "守护进程未运行": "Daemon not running",
  "无": "None",
  "服务状态": "Service status",
  "版本": "Version",
  ...
  "渠道": "Channels",
  "令牌": "Tokens",
  "Docker Compose 与连接": "Docker Compose and connection",
  "路径与 API 地址独立保存": "Paths and API address are saved independently",
  "项目目录": "Project directory",
  "Compose 文件": "Compose file",
  "Docker 可执行文件": "Docker executable",
  "API 服务名": "API service name",
  "选择 NewAPI 项目目录": "Select NewAPI project directory",
  "选择 Docker Compose 文件": "Select Docker Compose file",
  "选择 Docker 可执行文件": "Select Docker executable",
  "API 连接": "API connection",
  "已添加供应商": "Provider added",
  "未添加供应商": "Provider not added",
  "复制 Base URL": "Copy Base URL",
  "隐藏 API Key": "Hide API Key",
  "显示 API Key": "Show API Key",
  "复制 API Key": "Copy API Key",
  "隐藏 Management Key": "Hide Management Key",
  "显示 Management Key": "Show Management Key",
  "复制 Management Key": "Copy Management Key",
  "保存连接密钥": "Save connection keys",
  "用户 API Token": "User API token",
  "隐藏 API Token": "Hide API token",
  "显示 API Token": "Show API token",
  "复制 API Token": "Copy API token",
  "保存 API Token": "Save API token",
  "模型与调用": "Models and requests",
  "尚未读取模型": "Models not loaded",
  "刷新模型": "Refresh models",
  "测试中": "Testing",
  "测试 API": "Test API",
  "Codex++ 接入": "Codex++ integration",
  "NewAPI 内部聚合保持独立": "NewAPI's internal aggregation stays independent",
  "将 NewAPI 保存为一个受管 API 供应商，模型与渠道调度由 NewAPI 自身维护。":
    "Save NewAPI as one managed API provider while NewAPI maintains its own models and channel routing.",
  "系统名称": "System name",
  "初始化": "Setup",
  "已完成": "Completed",
  "未完成": "Incomplete",
  "受管 CLIProxyAPI 接入不会加入聚合供应商": "Managed CLIProxyAPI integrations do not join aggregate providers",
  "CLIProxyAPI 接入": "CLIProxyAPI integration",
  "将 CLIProxyAPI 作为受管直连供应商接入，包含官方和其他模型，不参与聚合调度。":
    "Connect CLIProxyAPI as a managed direct provider for official and other models. It is excluded from aggregate routing.",
  "CLIProxyAPI 官方登录": "CLIProxyAPI official sign-in",
  "在官方混合登录的模型列表中，于原生官方模型下方显示 CLIProxyAPI:模型名。":
    "In official mixed login, show CLIProxyAPI:model below native official models.",
  "CLIProxyAPI 未返回可接入模型，请先在管理页面完成账号登录后刷新模型。":
    "CLIProxyAPI did not return models to integrate. Complete account sign-in in management, then refresh models.",
  "请先启用 CLIProxyAPI 接入。": "Enable CLIProxyAPI integration first.",
  "Codex++ 接入通道": "Codex++ integration channels",
  "官方模型直连；其他模型可作为普通中转": "Official models are direct; other models can be regular relays",
  "CLIProxyAPI 官方模型": "CLIProxyAPI official models",
  "启用后会在官方原生模型下方增加 CLIProxyAPI:模型名；该专用通道不进入供应商列表或聚合调度。":
    "When enabled, CLIProxyAPI:model entries appear below native official models. This dedicated channel is excluded from the provider list and aggregate routing.",
  "请先在 CLIProxyAPI 页面启动服务并刷新模型，才能启用官方模型通道。":
    "Start CLIProxyAPI and refresh its models on the CLIProxyAPI page before enabling the official model channel.",
  "CLIProxyAPI 未返回可信官方 Codex 模型，请先在 CLIProxyAPI 管理页面完成账号登录后刷新模型。":
    "CLIProxyAPI did not return trusted official Codex models. Complete account sign-in in CLIProxyAPI management, then refresh models.",
  "未启用": "Disabled",
  "启用后仅处理可信官方 Codex 模型；它不会出现在供应商列表或聚合成员中，会在官方原生模型下方显示为 CLIProxyAPI:模型名。":
    "When enabled, this handles only trusted official Codex models. It is excluded from the provider list and aggregate members, and appears below native official models as CLIProxyAPI:model.",
  "更新 CLIProxyAPI 官方模型": "Update CLIProxyAPI official models",
  "启用 CLIProxyAPI 官方模型": "Enable CLIProxyAPI official models",
  "官方 Codex API 通道": "Official Codex API channel",
  "通用中转通道": "General relay channel",
  "已接入": "Connected",
  "未接入": "Not connected",
  "仅使用 CLIProxyAPI 返回的可信官方 Codex 模型，以 API 直连方式调用，不参与聚合调度。":
    "Uses only trusted official Codex models returned by CLIProxyAPI through direct API calls and never enters aggregate routing.",
  "未发现可信官方 Codex 模型": "No trusted official Codex models found",
  "接入官方 API 通道": "Connect official API channel",
  "承载 CLIProxyAPI 接入的其他模型，可作为普通 API 供应商加入现有聚合。":
    "Carries other models exposed by CLIProxyAPI and can join existing aggregates as a regular API provider.",
  "未发现可用于通用中转的其他模型": "No other models available for the general relay channel",
  "接入通用中转通道": "Connect general relay channel",
  "添加或更新供应商": "Add or update provider",
  "安装目录": "Install directory",
  "启动与连接位置": "Launch and connection settings",
  "可执行文件": "Executable",
  "选择 CLIProxyAPI 安装目录": "Select CLIProxyAPI install directory",
  "选择 CLIProxyAPI 可执行文件": "Select CLIProxyAPI executable",
  "选择 CLIProxyAPI 配置文件": "Select CLIProxyAPI configuration file",
  "保存启动与连接位置": "Save launch and connection settings",
  "修改启动位置前请先停止 Manager 启动的服务": "Stop the Manager-started service before changing launch settings",
  "请在 CLIProxyAPI 页面管理": "Manage this provider on the CLIProxyAPI page",
  "请在 NewAPI 页面管理": "Manage this provider on the NewAPI page",
  "官方账号登录": "Official account login",
  "刷新官方账号": "Refresh official account",
  "忽略当前 auth.json 的身份冲突并继续切换？": "Ignore the identity conflict in the current auth.json and continue switching?",
  "切换官方账号": "Switch official account",
  "删除官方账号": "Delete official account",
  "导入官方账号": "Import official accounts",
  "导出官方账号加密备份": "Export encrypted official account backup",
  "导出官方账号": "Export official accounts",
  "导出密码至少需要 8 个字符。": "The export password must be at least 8 characters.",
  "官方账号库": "Official account library",
  "正在读取官方账号": "Loading official accounts",
  "浏览器登录": "Browser login",
  ...
  "选择用于导出": "Select for export",
  "未命名账号": "Unnamed account",
  "分组": "Group",
  "标签": "Tags",
  "排序": "Order",
  "切换": "Switch",
  "刷新令牌与用量": "Refresh token and usage",
  "尚未保存官方账号": "No official accounts saved",
  "使用浏览器登录、设备码登录，或导入现有 auth.json。": "Use browser login, device code login, or import an existing auth.json.",
  "导出加密备份": "Export encrypted backup",
  "导入普通 auth.json 时密码可以留空；导入加密备份时请输入备份密码。": "Leave the password blank when importing a normal auth.json. Enter the backup password for an encrypted backup.",
  "备份使用 Argon2id 和 AES-256-GCM 加密，密码至少 8 个字符。": "Backups are encrypted with Argon2id and AES-256-GCM. Use a password of at least 8 characters.",
  "备份密码": "Backup password",
  "选择文件并导入": "Choose files and import",
  "选择位置并导出": "Choose location and export",
  "请在打开的页面输入设备码": "Enter the device code on the page that opened",
  "请在登录窗口完成 OpenAI 登录": "Complete the OpenAI login in the login window",
  "登录成功，账号已安全保存": "Login succeeded and the account was saved securely",
  "登录已取消": "Login cancelled",
  "登录失败，请重试": "Login failed. Try again.",
  "用量未查询": "Usage not checked",
  "用量查询失败": "Usage check failed",
  "用量已更新": "Usage updated",
};
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "删除官方账号“{0}”？此操作会同时删除本机保存的加密登录信息。": "Delete official account “{0}”? This also deletes its locally encrypted login data.",
  "认证顺序：{0} → {1}。": "Authentication order: {0} → {1}.",
  "官方认证已前置：{0}。该账号不会成为聚合成员，第三方成员的 API 将覆写实际请求。":
    "Official authentication is first: {0}. This account is not an aggregate member; third-party member APIs override actual requests.",
  "账号“{0}”已保存。": "Account “{0}” was saved.",
  "{0} 个账号；登录信息由本机当前用户加密保护": "{0} account(s); login data is encrypted for the current local user",
  "刷新于 {0}": "Refreshed at {0}",
  "主额度已用 {0}%": "Primary limit used: {0}%",
  "作者：{0} · {1}": "Author: {0} · {1}",
