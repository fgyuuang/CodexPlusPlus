...
fn sqlite_provider_thread_ids(path: &Path) -> anyhow::Result<HashMap<String, HashSet<String>>> {
    ...
    let db = Connection::open(path)?;
    let mut provider_threads: HashMap<String, HashSet<String>> = HashMap::new();
    for (table, id_column) in [("threads", "id"), ("local_thread_catalog", "thread_id")] {
        let columns = table_columns(&db, table)?;
        if !columns.contains(id_column) || !columns.contains("model_provider") {
            continue;
        }
        ...
        let mut stmt = db.prepare(&format!(
            "SELECT {id_column}, COALESCE(model_provider, '') FROM {table} WHERE COALESCE({id_column}, '') <> '' AND COALESCE(model_provider, '') <> ''{subagent_filter}"
        ))?;
