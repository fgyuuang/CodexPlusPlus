...
"#,
    ...
    #[test]
    fn ensure_marketplace_config_preserves_existing_user_marketplaces_without_reserved_writes() {
        let temp = tempfile::tempdir().unwrap();
        let home = temp.path();
        ...
        // 用户手工配置的保留名条目不属于我们的托管路径，不能覆盖或删除。
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some("\\?\\C:\\Users\\test\\old-path")
        );
        // 不再写入 Codex 会静默忽略的 openai-* 托管 marketplace。
        assert!(parsed["marketplaces"].get("openai-api-curated").is_none());
    }
